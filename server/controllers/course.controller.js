...
export const addLectureToCourse = catchAsync(async (req, res) => {
  ...
  try {
    ...
    throw new AppError(`Error uploading video: ${uploadError.message}`, 500);
  }
  if (!result || !result.secure_url) {
    throw new AppError("Error uploading video: No URL returned from server", 500);
  }
...
export const publishCourse = catchAsync(async (req, res) => {
  ...
  // Validate course has required fields and at least one lecture
  if (!course.title || !course.category || !course.price || !course.thumbnail) {
    throw new AppError("Course must have title, category, price, and thumbnail before publishing", 400);
  }
