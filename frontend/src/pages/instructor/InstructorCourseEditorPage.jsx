...
function InstructorCourseEditorPage() {
  ...
  const handlePublishCourse = async () => {
    if (!courseId || courseId === 'new') {
      setError('Please save the course first before publishing');
      return;
    }
    
    if (lectures.length === 0) {
      setError('Please add at least one lecture before publishing');
      return;
    }
    ...
    setError('');
    try {
      const res = await api.patch(`/course/c/${courseId}/publish`);
        ...
        <section className="section">
          <div className="page-header">
            <div>
              ...
              <p>Make your course visible to students by publishing it.</p>
            </div>
            <button
              className="btn-primary"
              ...
              style={{ 
                backgroundColor: course?.isPublished ? '#4CAF50' : '#ff6b6b'
              }}
            >
              {publishing ? 'Publishing…' : course?.isPublished ? '✓ Published' : 'Publish Course'}
            </button>
          </div>
          {course?.isPublished && (
            <p style={{ color: '#4CAF50', fontWeight: 'bold' }}>
              ✓ This course is published and visible to all students.
            </p>
          )}
        </section>
      )}
